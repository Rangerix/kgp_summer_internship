...
//this file is able to handle alphabets appended with residue number
#include <bits/stdc++.h>
using namespace std;
...
void buildchain(char filename[],char outfilename[],char resultfile[])
{
	...
	//cout<<"alpharesflag : "<<alpharesflag<<endl;
	...
	ofs.open(outfilename,ios::app);
	ofs<<" "<<atomvec[0].resinum<<" "<<atomvec[atomvec.size()-1].resinum<<" "<< alpharesflag << endl;
	ofs.close();
	...
	for(i=0;i<atomvec.size()-1;i++){
		...
		if(atomvec[i+1].resinum==1+atomvec[i].resinum){//continuous or charactered resinum
			//cout<<atomvec[i].resiname;
			//cout<<i<<" "<<atomvec[i].resinum<<" case 1"<<endl;
			tmp=shorten(atomvec[i].resiname);
		}
		else if(atomvec[i+1].resinum==atomvec[i].resinum&&atomvec[i+1].alphares!='\0'){
			//cout<<i<<" "<<atomvec[i].resinum<<" case 2"<<endl;
			tmp=shorten(atomvec[i].resiname);	
		}
			...
			//cout<<i<<" "<<atomvec[i].resinum<<" case 3"<<endl;
			...
			//cout<<i<<" "<<atomvec[i].resinum<<" case 4"<<endl;
...
void comparefile(char *pdbid,char *fastafile, char *outfilename, char *resultfile)
{
	...
	while(ifout){
		...
		temp.start=start;
		temp.end=end;
		temp.alpharesflag=alpharesflag;
		temp.sequence=str2;
	...
	for(i=0;i<compvec1.size();i++){
		...
		ofs<<"start : "<<compvec1[i].start<<" end : "<<compvec1[i].end<<" alpharesflag : "<<alpharesflag<<endl;
		ofs<<"PDB sequence : \n";
