...
c
 ...
 999  format(f12.6)
     ...
     1               ok, start, wwaters, resok, aok, dorsa, oldr
...
c
     ...
     1     )
      ...
      if ( dorsa ) then
         open ( 
     1        unit = 3,       
     1        file = fname(1:i)//'.rsa',
     1        status = 'unknown'
     1        )         
      endif
      open ( 
     1     unit = 4,                                                 
     1     file = fname(1:i)//'.log',
     1     status = 'unknown'
     1     )
...
c
     ...
     1     )     
      ...
      write(4,'(a,i7)')' MAX RESIDUES  ',naxA
...
c
      ...
      do while ( readstring(1,card,ilen).ge.0 )
         ...
         if (  atype.eq.1.or.
     1        (atype.eq.2.and.hetas).or.
     1        (atype.eq.3.and.wwaters) )then
c     
...
c
            if ( last.ne.card(18:27) ) then
               last = card(18:27)
               num_res = num_res + 1
               if( num_res.gt.naxA) then
                  write(4,*)
     1                 ' ERROR - Maximum number of residues exceeded ',
     1                 naxA
                  write(4,'(a)')
     1                 ' Increase naxA in accall.pars and recompile'
                  STOP'SOLVA_ERROR: naxA exceeded'
               endif
               res = card(18:20)
               resnam(num_res)=last
...
c
 6          rads(nats) = vdw
            ...
            if ( fullo ) then
               read(card(55:60),999)occup(nats)
               read(card(61:66),999)bfact(nats)
            endif
...
c
     ...
     1        )
         ...
         do i = 1, resindex(nats)
            if(oldr)then
               write(3,150)
     1              rlab(rty(i)), resnam(i),
     1              (ressums(i,j,1),ressums(i,j,2),j=1,7)
            else
               write(3,151)
     1              rlab(rty(i)), resnam(i),
     1              ressums(i,1,1),ressums(i,1,2),               
     1              (ressums(i,j,1),ressums(i,j,2),j=4,7)               
            endif
         enddo
         write(3,154)
         do i = 1, num_chains
...
c----------------------------------------------------------       
 ...
 120  format('REM  File of summed (Sum) and % (per.)',
     1     ' accessibilities for ',a)

 125  format('REM RES _ NUM      All-atoms   Non-P-side  ',
     1     ' Polar-Side   Total-Side   Main-Chain    ',
     1     'Non-polar    All polar')
 126  format('REM RES _ NUM      All-atoms   Total-Side  ',
     1     ' Main-Chain    Non-polar    All polar')

 130  format('REM                ABS   REL    ABS   REL',
     1     '    ABS   REL    ABS   REL    ABS   REL',
     1     '    ABS   REL    ABS   REL')
...
c     --	ouput written to .rsa file (channel 4)      
     ...
     1              rtype(maxr),
     1              nacids
      real	    ressums(naxA,7,2),
     1              tsums(7), 
     1              accs(maxs),
     1		    standarea(maxr,7)
      character*256 line, sname
      character*10  resnam(naxA)
...
c
     ...
     1                  ' external file "',sname(1:slen),'"'
         ...
         enddo
         write(4,'(a,i3,a)')
     1        ' RELATIVE (STANDARD) ACCESSIBILITIES READFOR ',
     1        i,' AMINO ACIDS'
         close(1)
      else
